...
var tempAvges = {}; // average of temperatures for each year
...
function get_years() {// GET THE YEARS
...
function get_values(){// GET OUR VALUES
  var tableYears = table.getColumn('value');
    var uniqueValue = int_array(unique_array(tableYears));
    sort(uniqueValue);
    return uniqueValue;
}
...
function setup() {
  cnv = createCanvas(plotWidth + leftMargin*2,plotHeight + topMargin*2);
...
function draw() {
  ...
  text('TEMPERATURE AVERAGES', 600, 50); //TITLE POSITION
  ...
  rotate(-90);
  textAlign(CENTER);
  text('avg temp', 0 - (topMargin + plotHeight/2), leftMargin - 50);
  rotate(90);
   ...
   for (var i = 0 ; i < years.length ; i++) {
       var year = years[i];
       var avg;
       var temp = table.findRows('value' + year, 'year');
       console.log('temp for year ' + year, temp);
   ...
   console.log('Averages of temp per year', tempAvges);
   ...
   var pixelsPerHp = (plotHeight - surplus * 2) / (maxHp - minHp);
    ...
    for (var i = 0 ; i < years.length ; i++) {
        var year = years[i];
        var hpAboveMin = tempAvges[year] - minHp;
        var x = leftMargin + pixelsPerYear * (1+i);
    ...
    for (var i = 0 ; i < years.length ; i++) {
        ...
        if (mouseX > x - mouseMargin && mouseX < x + mouseMargin &&
            ...
            textAlign(CENTER);
            text('' + Math.round(tempAvges[years[i]]*100)/100, x + 45, y + 10);
            strokeWeight(7);
            point(x, y);
            strokeWeight(1);
        }
